...
#from utils.graph import build_graph, get_graph_info, save_graph, load_graph, load_graphml
...
class Net(nn.Module):
    def __init__(self, cfg):
        ...
        if (cfg["USE_PRUNED_GRAPH"] == True):
			      print("hi1")
            graph2 = load_graphml('./output/graph/conv2.graphml')
            graph3 = load_graphml('./output/graph/conv3.graphml')
            graph4 = load_graphml('./output/graph/conv4.graphml')
            graph5 = load_graphml('./output/graph/conv5.graphml')
            print(get_graph_info(graph2))
            print(get_graph_info(graph3))
            print(get_graph_info(graph4))
            print(get_graph_info(graph5))
            ...
            nx.draw_networkx(graph2, arrows=True, **options)
            plt.show()
            print(get_graph_info(graph2))
            # nx.draw_networkx(graph3, arrows=True, **options)
            # plt.show()
            # nx.draw_networkx(graph4, arrows=True, **options)
            # plt.show()
            # nx.draw_networkx(graph5, arrows=True, **options)
            # plt.show()
